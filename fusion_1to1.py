...
class TransformerAttentionFusion(nn.Module):
    def __init__(
        ...
        self.upsample = nn.Upsample(scale_factor=down_factor, mode='trilinear', align_corners=False)
...
def fuse_images(content_dir, style_images_dir, content_depths_dir, style_depths_dir, output_dir, 
    ...
    content_files = sorted([f for f in os.listdir(content_dir) if f.endswith(('.jpg', '.png', '.jpeg', '.bmp', '.tiff'))])
    style_files = sorted([f for f in os.listdir(style_images_dir) if f.endswith(('.jpg', '.png', '.jpeg', '.bmp', '.tiff'))])

    # Process each pair of content and style images
    for content_file, style_file in tqdm(zip(content_files, style_files), total=len(content_files)):
        # Load content image
        content_image = load_image(os.path.join(content_dir, content_file))
        content_base, content_ext = os.path.splitext(content_file)
        style_base, _ = os.path.splitext(style_file)
        
        # Load corresponding content depth map
        content_base_name = os.path.splitext(content_file)[0]  
        content_depth_path = find_matching_file(content_base_name, content_depths_dir)
        
        if content_depth_path is None:
            raise ValueError(f"Content depth map not found for {content_file} in {content_depths_dir}")
        
        # Read depth map as grayscale
        ...
        # Normalize depth map to [0, 1] and convert to tensor
        content_depth_map = torch.tensor(content_depth_map, dtype=torch.float32) / 255.0
        content_depth_map = content_depth_map.unsqueeze(0).unsqueeze(0).to(device) 

        # Load style image
        style_image_path = os.path.join(style_images_dir, style_file)
        style_image = load_image(style_image_path)
        style_base, _ = os.path.splitext(style_file)
            
        # Load corresponding style depth map
        style_base_name = os.path.splitext(style_file)[0] 
        style_depth_path = find_matching_file(style_base_name, style_depths_dir)
        ...
        style_img_cv = cv2.imread(style_image_path)
        if style_img_cv is None:
            raise ValueError(f"Style image not found: {style_image_path}")
        ...
        # Create a waterbody image with the same size as the style image
        waterbody_img = np.ones_like(style_img_cv, dtype=np.float32) * waterbody_color.reshape(1, 1, 3)
        # Convert to 8-bit image before converting to tensor
        waterbody_img_uint8 = np.clip(waterbody_img * 255.0, 0, 255).astype(np.uint8)

        # Convert to a PIL image and then to a tensor (using same preprocessing as load_image)
        ...
        # Fuse features at each level using **content depth maps** for blending
        if use_multi_scale:
        ...
        # Decode with depth-based skip connections
        ...
        output_filename = f"{content_base}_stylized_with_{style_base}.jpg"  
...
if __name__ == "__main__":
    ...
    content_dir = os.path.join(base_dir, "inputs/1to1/content/images")
    content_depths_dir = os.path.join(base_dir, "inputs/1to1/content/depths")
    style_images_dir = os.path.join(base_dir, "inputs/1to1/style/images")
    style_depths_dir = os.path.join(base_dir, "inputs/1to1/style/depths")  
    output_dir = os.path.join(base_dir, "outputs_1to1")
